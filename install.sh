...
if [[ $( echo $? ) == 0 ]]; then
  ...
  export OPTIONS=("Kill PID on port 55555" "Exit the script")
  bashSelect
  case $? in
    0 )
      status "killing PID on 55555"
      runCommand "apt -y install psmisc"
	  runCommand "fuser -4 55555/tcp -k"
      ;;
    1 )
      exit 0
      ;;
...
bold="\e[1m"
reset="\e[0m"
port=\$(lsof -Pi :55555 -sTCP:LISTEN -t)
if [ -z "\$port" ]; then
...
if [[ -z "$port" ]]; then
    ...
    txadmin="http://$(ifconfig eth0 | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*' | grep -v '127.0.0.1'):55555"
